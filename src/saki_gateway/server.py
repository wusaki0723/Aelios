...
class GatewayApp:
    ...
    def _synthesize_companion_reply(
        ...
        *,
        ...
        response = request_chat_completion(
            chat_provider,
            self._build_main_chat_messages(messages, tool_contexts, session_id, profile_id),
            stream=False,
            temperature=temperature,
        )
        content = extract_text_content(response).strip()
    ...
    def _build_main_chat_messages(
        ...
        important_memories = [
            record
            for record in self.memory_store.list_memories(limit=200, memory_kind="long_term")
            if getattr(record, "category", "") != "memory_refresh"
    ...
    def clear_long_term_memories(self) -> Dict[str, Any]:
        deleted = self.memory_store.delete_memories(["long_term"])
        self._ensure_context_files(refresh=True)
        return {
            "success": True,
            "deleted": deleted,
    ...
    def clear_daily_logs(self) -> Dict[str, Any]:
        deleted = self.memory_store.delete_memories(["daily_log"])
        self._ensure_context_files(refresh=True)
        return {
            "success": True,
            "deleted": deleted,
            "memory_kinds": ["daily_log"],
        }

    def run_memory_digest_payload(self, body: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        payload = body or {}
        profile_id = str(payload.get("profile_id", "local-user") or "local-user").strip() or "local-user"
        session_id = str(payload.get("session_id", "") or "").strip()
        day_start, _ = self._local_day_bounds()
        log_id = self._daily_log_memory_id(profile_id, day_start)
        daily_log = self.memory_store.get_memory(log_id)
        if daily_log is None or not str(daily_log.content or "").strip():
            return {
                "success": False,
                "message": "还没有可用于整理的今日日志",
                "updated": 0,
                "created": 0,
            }
        ...
        before_records = {
            item.id: item
            for item in self.memory_store.list_memories(limit=5000, memory_kind="long_term")
        }
        before_visible_ids = {
            item.id for item in before_records.values() if getattr(item, "category", "") != "memory_refresh"
        }
        before_refresh_ids = {
            item.id for item in before_records.values() if getattr(item, "category", "") == "memory_refresh"
        }

        self._run_memory_digest(profile_id=profile_id, session_id=session_id)
        ...
        after_visible = [
            item for item in after_records.values() if getattr(item, "category", "") != "memory_refresh"
        ]
        created = 0
        updated = 0
        changed_items: list[Dict[str, Any]] = []
        for item in after_visible:
            previous = before_records.get(item.id)
            if previous is None:
                created += 1
                changed_items.append(self._serialize_memory(item))
                continue
            if (
                previous.key != item.key
                or previous.content != item.content
                or previous.category != item.category
                or float(previous.importance or 0.0) != float(item.importance or 0.0)
            ):
                updated += 1
                changed_items.append(self._serialize_memory(item))

        refresh_marker_created = any(
            item_id not in before_refresh_ids and getattr(item, "category", "") == "memory_refresh"
        ...
        return {
            ...
            "refresh_marker_created": refresh_marker_created,
            "processed_today_log": True,
            "items": changed_items[:20],
        }
    ...
    def get_context_payload(self, profile_id: str = "", session_id: str = "") -> Dict[str, Any]:
        ...
        return {
            ...
            "persona": self._read_text_file(self._core_memory_file()),
            "important_long_term_memories": important_memories,
            "recent_messages": recent_messages[-50:],
            "today_daily_log": daily_log.content if daily_log else "",
    ...
    def _find_dashboard_root(self) -> Optional[Path]:
        ...
        candidates = [
            package_static,
            self.root / "src" / "saki_gateway" / "static",
    ...
    def do_POST(self) -> None:
        try:
            ...
            if parsed.path == "/api/memories":
                ...
                return
            if parsed.path == "/api/memories/digest":
                self._json(HTTPStatus.OK, app.run_memory_digest_payload(body))
                return
            if parsed.path == "/api/reminders":
    ...
    def do_DELETE(self) -> None:
        try:
            ...
            if parsed.path == "/api/memories":
                ...
                return
            if parsed.path == "/api/memories/long-term":
                self._json(HTTPStatus.OK, app.clear_long_term_memories())
                return
            if parsed.path == "/api/logs":
                self._json(HTTPStatus.OK, app.clear_daily_logs())
                return
            if parsed.path.startswith("/api/memories/"):
                memory_id = parsed.path.rsplit("/", 1)[-1]
